...
import { Users, BookOpen, Wallet, Trophy, Loader2, AlertTriangle, CheckCircle2, TrendingUp, Star, Trash2, Zap, Settings, ShieldCheck, MailCheck } from 'lucide-react';
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ onNavigate, requests, onApprove }) => {
  ...
  useEffect(() => {
    ...
    return () => { unsubUsers(); unsubSpecial(); };
  }, []);
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-20 font-['Hind_Siliguri']">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard icon={<Users/>} label="মোট ইউজার" value={stats.totalUsers.toString()} color="bg-blue-600" />
        <StatCard icon={<BookOpen/>} label="পেইড কুইজ" value={stats.totalQuizzes.toString()} color="bg-indigo-600" />
        <StatCard icon={<TrendingUp/>} label="মোট ব্যালেন্স" value={`৳${stats.totalRevenue}`} color="bg-emerald-600" />
        <StatCard icon={<Trophy/>} label="পেন্ডিং রিকোয়েস্ট" value={requests.filter(r => r.status === 'pending').length.toString()} color="bg-amber-500" />
      </div>

      {/* Setup Checklist for Firebase (Critical for user request) */}
...
const StatCard = ({ icon, label, value, color }: any) => (
  <div className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm flex items-center gap-6 hover:translate-y-[-4px] transition-all">
    <div className={`w-14 h-14 ${color} text-white rounded-[20px] flex items-center justify-center shadow-lg`}>
