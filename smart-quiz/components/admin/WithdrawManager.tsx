...
import { CreditCard, Check, X, Clock, User, Phone, Wallet, AlertCircle } from 'lucide-react';
...
const WithdrawManager: React.FC<WithdrawManagerProps> = ({ requests, onApprove, onReject }) => {
      ...
      <div className="grid grid-cols-1 gap-6">
          ...
          <div key={req.id} className="bg-white p-8 rounded-[44px] shadow-sm border border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-8 group hover:border-emerald-200 transition-all">
            ...
            <div className="flex gap-3">
               <button 
                ...
                className="flex-1 md:flex-none px-8 py-4 bg-rose-50 text-rose-600 rounded-2xl font-black text-sm hover:bg-rose-600 hover:text-white transition-all active:scale-95 flex items-center justify-center gap-2"
               >
                 <X size={18} /> রিজেক্ট
               </button>
               <button 
                onClick={() => onApprove(req.id)}
                className="flex-1 md:flex-none px-8 py-4 bg-emerald-700 text-white rounded-2xl font-black text-sm shadow-xl shadow-emerald-700/20 hover:bg-emerald-800 transition-all active:scale-95 flex items-center justify-center gap-2"
               >
        ...
        {pendingRequests.length === 0 && (
          <div className="py-24 text-center bg-white rounded-[50px] border border-dashed border-slate-200">
             ...
             <p className="text-slate-300 font-black uppercase tracking-widest text-xs">কোনো উইথড্র রিকোয়েস্ট নেই</p>
          </div>
        )}
      </div>
    </div>
