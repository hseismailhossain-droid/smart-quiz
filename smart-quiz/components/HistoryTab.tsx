...
import { Bookmark, AlertCircle, CheckCircle2, Trophy, Clock, HelpCircle, ArrowRight, XCircle } from 'lucide-react';
...
const HistoryTab: React.FC<HistoryTabProps> = ({ history }) => {
  ...
  const mistakes = Array.from(new Set(exams.flatMap(e => e.mistakes || []).map(m => JSON.stringify(m)))).map(s => JSON.parse(s));
  ...
  const renderContent = () => {
          ...
          {exams.map((res) => (
            <div key={res.id} className="bg-white p-6 rounded-[32px] border border-slate-100 shadow-sm flex justify-between items-center group hover:border-emerald-200 transition-all">
              <div className="flex gap-5 items-center">
                ...
                <div>
                  <h4 className="font-black text-slate-800 text-sm leading-tight mb-1 truncate max-w-[150px]">{res.subject || 'সাধারন জ্ঞান'}</h4>
    ...
    if (activeSubTab === 'mistakes') {
      return mistakes.length > 0 ? (
        <div className="space-y-6 p-5 pb-24 animate-in fade-in slide-in-from-bottom-4 duration-500">
          {mistakes.map((q, idx) => (
            <div key={idx} className="bg-white p-6 rounded-[36px] border border-rose-100 shadow-sm relative overflow-hidden group">
              <div className="absolute top-0 right-0 w-16 h-16 bg-rose-50 rounded-bl-[40px] flex items-center justify-center text-rose-300 group-hover:text-rose-500 transition-colors">
                <XCircle size={20} />
              </div>
              ...
              <div className="flex items-start gap-4 mb-6">
                ...
                <h4 className="font-black text-slate-800 text-sm leading-relaxed pt-1 pr-10">{q.question}</h4>
              </div>
              ...
              <div className="space-y-3">
                <div className="p-4 bg-emerald-50 rounded-2xl flex items-center gap-3 border border-emerald-100">
                  ...
                  <div>
                    <p className="text-[9px] font-black text-emerald-600 uppercase tracking-widest">সঠিক উত্তর</p>
                    <span className="text-xs font-black text-emerald-900">{q.options ? q.options[q.correctAnswer] : 'লোড হচ্ছে...'}</span>
                  </div>
                </div>
                {q.explanation && (
                  <div className="p-4 bg-blue-50/50 rounded-2xl border border-blue-50">
                    <p className="text-[9px] font-black text-blue-600 uppercase mb-1">ব্যাখ্যা</p>
                    <p className="text-[11px] font-bold text-slate-600 leading-relaxed">{q.explanation}</p>
                  </div>
                )}
              </div>
            </div>
          ...
          {marked.map((q, idx) => (
            <div key={idx} className="bg-white p-6 rounded-[32px] border border-emerald-100 shadow-sm">
              ...
              <div className="grid grid-cols-1 gap-3">
                {q.options?.map((opt, i) => (
                  <div key={i} className={`p-4 rounded-2xl text-xs font-black border flex items-center justify-between ${i === q.correctAnswer ? 'bg-emerald-700 text-white border-emerald-700 shadow-lg' : 'bg-slate-50 text-slate-400 border-slate-50'}`}>
                    {opt}
  ...
  return (
    <div className="flex flex-col h-full bg-white font-['Hind_Siliguri'] overflow-hidden">
      <div className="bg-white px-4 pt-8 pb-4 shrink-0">
        <h2 className="text-3xl font-black text-slate-900 mb-6 px-4 tracking-tight">হিস্ট্রি ও প্র্যাকটিস</h2>
        <div className="flex justify-around bg-slate-50 p-1.5 rounded-[28px] mx-2 shadow-sm border border-slate-100">
...
const HistorySubTab = ({ label, active, onClick }: { label: string, active: boolean, onClick: () => void }) => (
  <button 
    onClick={onClick}
    className={`flex-1 py-3 text-[11px] font-black uppercase tracking-widest rounded-[22px] transition-all duration-300 ${active ? 'bg-white text-emerald-700 shadow-sm shadow-emerald-700/10' : 'text-slate-400 hover:text-slate-600'}`}
  >
    {label}
  </button>
);
...
const EmptyState = ({ label, icon }: { label: string, icon: React.ReactNode }) => (
  <div className="flex flex-col items-center justify-center text-center p-12 h-full opacity-80 animate-in fade-in zoom-in duration-500 min-h-[300px]">
