...
const HistoryTab: React.FC<HistoryTabProps> = ({ history }) => {
  ...
  return (
    <div className="flex flex-col min-h-full bg-white font-['Hind_Siliguri']">
      <div className="bg-white px-4 pt-8 sticky top-0 z-20">
        <h2 className="text-3xl font-black text-slate-900 mb-8 px-4 tracking-tight">হিস্ট্রি ও প্র্যাকটিস</h2>
...
const HistorySubTab = ({ label, active, onClick }: { label: string, active: boolean, onClick: () => void }) => (
  <button 
    onClick={onClick}
    className={`flex-1 py-3 text-[11px] font-black uppercase tracking-widest rounded-[22px] transition-all duration-300 ${active ? 'bg-white text-emerald-700 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
  >
    {label}
  </button>
);
