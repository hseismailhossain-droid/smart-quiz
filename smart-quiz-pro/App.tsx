...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const handleError = (event: PromiseRejectionEvent) => {
      ...
      if (reason.includes('IndexedDB')) refreshFirestore();
    };
        ...
        if (firebaseUser) {
          ...
          const unsubUser = onSnapshot(doc(db, 'users', firebaseUser.uid), (docSnap) => {
            if (docSnap.exists()) {
              ...
              if (view === 'loading' || view === 'auth' || view === 'setup') setView(isUserAdmin ? 'admin' : 'main');
            } else {
          ...
          const unsubNotif = onSnapshot(query(collection(db, 'notifications'), limit(20)), (snapshot) => {
          ...
          // Corrected History Listener: Use higher limit and better memory sort for latency compensation
          const unsubExams = onSnapshot(
            ...
            (snap) => {
              ...
              // Improved Sort: Handle null server timestamps by placing them at the top
  ...
  if (view === 'error') return (
    <div className="min-h-screen items-center justify-center bg-white flex flex-col p-10 text-center font-['Hind_Siliguri']">
       ...
       <button onClick={() => refreshFirestore()} className="bg-emerald-700 text-white px-8 py-4 rounded-2xl font-black flex items-center gap-2 shadow-xl active:scale-95 transition-all">
